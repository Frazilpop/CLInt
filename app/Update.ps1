...
# User data (the data\ folder: settings.json, tdp-settings.json,
# menu-key.txt, recent.json...) is never in the download, so it survives
# updates untouched.
#
...
$here = $PSScriptRoot                  # app\ (or a flat test copy)
$root = if ((Split-Path $here -Leaf) -eq 'app') { Split-Path $here -Parent } else { $here }
$repo = 'Frazilpop/CLInt'
$dataDir = Join-Path $root 'data'
New-Item -ItemType Directory -Force $dataDir | Out-Null
$marker = Join-Path $dataDir 'update-available.txt'

function Get-LocalVersion {
    try { (Get-Content (Join-Path $root 'version.txt') -TotalCount 1).Trim() } catch { '0.0.0' }
}
...
if ((Test-Path (Join-Path $root '.git')) -and $git) {
    ...
    $remote = (& git -C $root rev-parse '@{u}').Trim()
    if ($local -eq $remote) {
    ...
    if ($CheckOnly) {
        $rv = ''
        try { $rv = ([string](& git -C $root show 'origin/main:version.txt' 2>$null | Select-Object -First 1)).Trim() } catch {}
...
try {
    ...
    $zipRoot = Get-ChildItem $tmp -Directory | Select-Object -First 1
    Copy-Item (Join-Path $zipRoot.FullName '*') $root -Recurse -Force
    # A ZIP overlay only adds files; sweep ones old versions shipped under
    # names that have since been renamed away, plus root copies of scripts
    # that moved into app\ in v0.2.10.
    foreach ($legacy in @('SteamMenu.ps1', 'SteamMenuKey.ahk', 'Backdrop.ps1',
                          'Install.ps1', 'Uninstall.ps1', 'Update.ps1', 'CLIntKey.ahk', 'CLInt.ico')) {
